...
class Viewer:
    ...
    async def send_game_result(self, winner_player_number: PlayerNumber | None, context: GameContext):
        await self.send(
            SendGameResult(
                winner_player_number=winner_player_number.value if winner_player_number is not None else 0
            )
        )
